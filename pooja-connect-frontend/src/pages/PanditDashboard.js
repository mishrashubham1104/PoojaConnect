...
const PanditDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [profileRes, bookingsRes] = await Promise.all([
        axios.get(`https://pooja-backend.onrender.com/api/pandits/my-profile/${panditId}`),
        axios.get(`https://pooja-backend.onrender.com/api/bookings/pandit/${panditId}`)
      ]);
  ...
  const updateStatus = async (bookingId, newStatus, userContext) => {
    try {
      await axios.put(`https://pooja-backend.onrender.com/api/bookings/${bookingId}`, {
        status: newStatus,
